...
#include "fe_shape_function_1d.h"
  ...
  for ( ; el != end_el ; ++el)
    {
      ...
      // create new class instances and copy value over
      fe_shape_function<Real> fe_phi(phi.size(), qrule.n_points());
      fe_shape_function<RealGradient> fe_dphi(dphi.size(), qrule.n_points());
      
      for (int qp=0; qp<prule.n_points(); qp++) {
          for (int i=0; i<phi.size(); i++)
            fe_phi[i][qp] = phi[i][qp];
            
          for (int j=0; j<dphi.size(); j++)
            fe_dphi[j][qp] = dphi[j][qp];
      }
      ...
      START_LOG("assemble_poisson()","QuadraturePointLoop");
      ...
      for (unsigned int qp=0; qp<qrule.n_points(); qp++)
        {
          ...
          for (unsigned int i=0; i<fe_phi.size(); i++)
            for (unsigned int j=0; j<fe_phi.size(); j++)
              {
                Ke(i,j) += JxW[qp]*(fe_dphi[i][qp]*fe_dphi[j][qp]);
              }
          ...
          // "forcing function" in the PDE against the test functions.
          {
            ...
            for (unsigned int i=0; i<fe_phi.size(); i++)
              Fe(i) += JxW[qp]*fxy*fe_phi[i][qp];
          }
        }
        
        STOP_LOG("assemble_poisson()","QuadraturePointLoop");
